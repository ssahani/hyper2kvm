    ...
    try:
        ...
        for idx, root in enumerate(roots, 1):
            ...
            if os_type == "windows" and (args.show_windows or args.show_all):
                ...
                try:
                    product = g.win_registry_read("SOFTWARE", r"Microsoft\Windows NT\CurrentVersion", "ProductName")
                    build = g.win_registry_read("SOFTWARE", r"Microsoft\Windows NT\CurrentVersion", "CurrentBuild")
                    edition = g.win_registry_read("SOFTWARE", r"Microsoft\Windows NT\CurrentVersion", "EditionID")
...
Requires:
  ...
  - hivex (for Windows registry parsing)
    ...
    parser.add_argument(
        'image',
        type=Path,
        help='Path to disk image (qcow2, raw, vmdk, vdi, etc.)'
    )
    ...
    parser.add_argument(
        '--show-security',
        action='store_true',
        help='Show security module detection (SELinux, AppArmor)'
    )

    parser.add_argument(
        '--show-packages',
        action='store_true',
        help='Show installed packages (RPM, APT, Pacman)'
    )

    parser.add_argument(
        '--show-windows',
        action='store_true',
        help='Show Windows-specific info (registry, users)'
    )

    parser.add_argument(
        '--show-services',
        action='store_true',
        help='Show Linux systemd services'
    )

    parser.add_argument(
        '--show-filesystems',
        action='store_true',
        help='Show filesystems and partitions'
    )

    parser.add_argument(
        '--show-performance',
        action='store_true',
        help='Show performance metrics and cache statistics'
    )
