...
find_charts() {
    log_info "Searching for Helm charts..."
    ...
    log_success "Found ${#charts[@]} chart(s): ${charts[*]}"
    echo "${charts[@]}"
}
...
main() {
    ...
    echo "======================================"
    echo -e "${GREEN}Packaged:${NC} $packaged chart(s)"
    echo -e "${RED}Failed:${NC}   $failed chart(s)"
