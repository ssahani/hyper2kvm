...
if [ -f "$WEBHOOK_CONFIG" ]; then
    ...
    sed "s/\${CA_BUNDLE}/$CA_BUNDLE/g" "$WEBHOOK_CONFIG" > "$TMP_DIR/webhook-config-updated.yaml"

    echo "Updated webhook configuration saved to: $TMP_DIR/webhook-config-updated.yaml"
    echo ""
    echo "To apply the webhook configuration:"
    echo "  kubectl apply -f $TMP_DIR/webhook-config-updated.yaml"
    echo ""

    # Optionally apply directly
    read -p "Apply webhook configuration now? (yes/no): " APPLY
    if [ "$APPLY" == "yes" ]; then
        kubectl apply -f "$TMP_DIR/webhook-config-updated.yaml"
        echo "Webhook configuration applied!"
    fi
else
    echo "WARNING: Webhook config file not found: $WEBHOOK_CONFIG"
