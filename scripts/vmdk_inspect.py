...
def apply_auto_fix(info: VMDKInfo, output_path: str) -> bool:
    ...
    print(f"   (This may require sudo for libguestfs operations)\n")

    try:
        result = subprocess.run(
            ["hyper2kvm", "--config", config_file],
            text=True
        )
        ...
        if result.returncode == 0:
            ...
            if "requires root" in str(result.stderr) if hasattr(result, 'stderr') else "":
                print(f"\n💡 Try with sudo:")
...
def main():
    ...
    args = [a for a in sys.argv[1:] if a not in ("--json", "--no-fail", "--auto-fix", "--output") and not (output_file and a == output_file)]
