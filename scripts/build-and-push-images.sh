...
docker_login() {
    ...
    if [ -z "${GITHUB_TOKEN}" ]; then
        log_info "GITHUB_TOKEN not set, attempting interactive login..."
        echo "${GITHUB_TOKEN}" | docker login "${GHCR_REGISTRY}" -u "${GITHUB_USER}" --password-stdin 2>/dev/null || {
            log_error "Docker login failed. Set GITHUB_TOKEN environment variable"
