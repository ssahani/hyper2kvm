...
VMCraft is **5-10x faster** than libguestfs:

| Operation | libguestfs | VMCraft | Speedup |
|-----------|------------|---------|---------|
| Launch | ~10-13s | ~1.9s | **5-7x** |
| NBD Connect | N/A (appliance boot) | ~1.4s | N/A |
| Inspection | ~2-3s | ~0.3s | **6-10x** |
| File Operations | Baseline | **2-5x faster** (with LRU cache) | **2-5x** |
...
|---------|------------------|-------------------|
| **Backend** | ComprehensiveGuestInspector (libguestfs) | VMCraft (NBD-based) |
| **Performance** | Slower (~10-13s launch) | **5-10x faster** (~1.9s launch) |
...
- libguestfs: Original disk image manipulation library
