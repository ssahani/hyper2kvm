...
test_prerequisites() {
    ...
    K8S_VERSION=$(kubectl version --short 2>/dev/null | grep Server | awk '{print $3}' || kubectl version -o json 2>/dev/null | grep gitVersion | head -1 | awk -F'"' '{print $4}')
      ...
      - name: create-vmdk
        ...
        command:
          ...
          - |
            ...
            dd if=/dev/zero of=test-disk.raw bs=1M count=$TEST_VM_SIZE status=progress
