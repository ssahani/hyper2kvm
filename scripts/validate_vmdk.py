...
Note:
  ...
  - Full mode may take several minutes for large disks
    ...
    parser.add_argument(
        'image',
        type=Path,
        help='Path to disk image (VMDK, QCOW2, VHD, etc.)'
    )

    parser.add_argument(
        '--deep',
        action='store_true',
        help='Enable deep validation (partition table check via NBD)'
    )

    parser.add_argument(
        '--full',
        action='store_true',
        help='Enable full validation (includes read-only fsck)'
    )
    ...
    parser.add_argument(
        '--json',
        action='store_true',
        help='Output results as JSON (for --deep/--full only)'
    )
