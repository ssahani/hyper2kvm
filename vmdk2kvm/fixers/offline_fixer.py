...
class OfflineFSFix:
    ...
    def mount_root_bruteforce(self, g: guestfs.GuestFS) -> None:
        ...
        if not parts:
            U.die(self.logger, "Failed to list partitions for brute-force mount.", 1)
    ...
    def remove_vmware_tools_func(self, g: guestfs.GuestFS) -> Dict[str, Any]:
        """
        Uses libguestfs mount_local to expose the mounted guest filesystem at a temp dir,
        then runs OfflineVmwareToolsRemover against that tree.

        IMPORTANT: mount_local_run() can block; we only call it after mount_local succeeded
        and we always attempt umount_local() in finally.
        """
        ...
        try:
            try:
                g.mount_local(str(mnt))
                mounted_local = True
                g.mount_local_run()
            except Exception as e:
                res.errors.append(f"mount_local_failed:{e}")
                return res.as_dict()
    ...
    def create_validation_suite(self, g: guestfs.GuestFS) -> ValidationSuite:
        ...
        def check_boot_files(_context):
            for p in ("/boot", "/boot/grub", "/boot/grub2"):
                try:
                    if g.is_dir(p):
                        return True
                except Exception:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            cloud_init = getattr(self, "inject_cloud_init")(g) if hasattr(self, "inject_cloud_init") else {"enabled": False}
