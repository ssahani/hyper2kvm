...
# vmdk2kvm/fixers/windows_network_fixer.py
# -*- coding: utf-8 -*-
...
def _safe_logger(self) -> logging.Logger:
    ...
    if isinstance(lg, logging.Logger):
        return lg
    return logging.getLogger("vmdk2kvm.windows_network_fixer")
...
Log "=== vmdk2kvm network apply starting ==="
...
def retain_windows_network_config(self, g: guestfs.GuestFS) -> Dict[str, Any]:
    ...
    try:
        with _step(logger, "🧾 Stage network payload + PowerShell apply script"):
            _guest_mkdir_p(g, "/vmdk2kvm", dry_run=dry_run)
    ...
    extra_cmd = r'powershell.exe -ExecutionPolicy Bypass -NoProfile -File "C:\vmdk2kvm\net\apply-network.ps1"'
    with _step(logger, "🛠️ Provision firstboot hook for network apply"):
        try:
            fb = provision_firstboot_payload_and_service(
                ...
                service_name="vmdk2kvm-net-firstboot",
                guest_dir="/vmdk2kvm",
                log_path=f"{paths.temp_dir}/vmdk2kvm-net-firstboot.log",
                driver_stage_dir="/vmdk2kvm/drivers/virtio",  # may or may not exist; framework tolerates
                extra_cmd=extra_cmd,
    ...
    result["notes"] += [
        ...
        r"Artifacts: C:\vmdk2kvm\net\saved_network.json, C:\vmdk2kvm\net\apply-network.ps1",
        r"Optional override: C:\vmdk2kvm\net\network_override.json (if provided via windows_network_config[_path])",
        r"Log (inside Windows): C:\vmdk2kvm\net\apply-network.log",
