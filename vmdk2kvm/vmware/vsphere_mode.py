    ...
    def run(self) -> int:
        ...
        try:
            ...
            if action == "vm_disks":
                ...
                for idx, disk in enumerate(disks):
                    ...
                    disk_info = {
                        ...
                        "key": disk.key,
                        "capacity_gb": disk.capacityInBytes / (1024 ** 3) if hasattr(disk, 'capacityInBytes') else disk.capacityInKB / 1024 / 1024,
            ...
            if action == "download_datastore_file":
                ...
                dc_name = getattr(self.args, "dc_name", "ha-datacenter")
                chunk_size = int(getattr(self.args, "chunk_size", 1024 * 1024))
                try:
                    client.download_datastore_file(datastore=self.args.datastore, ds_path=self.args.ds_path, local_path=local_path, dc_name=dc_name, chunk_size=chunk_size)
                except VMwareError as e:
                    raise Fatal(2, f"vsphere download_datastore_file: {e}")
                output = {"status": "success", "local_path": str(local_path), "datastore": self.args.datastore, "ds_path": self.args.ds_path}
                if self.args.json:
            ...
            if action == "query_changed_disk_areas":
                ...
                try:
                    changed = client.query_changed_disk_areas(vm, snapshot=snapshot, device_key=device_key, start_offset=start_offset, change_id=change_id)
                except Exception as e:
            ...
            if action == "download_vm_disk":
                ...
                chunk_size = int(getattr(self.args, "chunk_size", 1024 * 1024))
                try:
                    client.download_datastore_file(datastore=datastore, ds_path=ds_path, local_path=local_path, dc_name=dc_name, chunk_size=chunk_size)
                except VMwareError as e:
                    raise Fatal(2, f"vsphere download_vm_disk: {e}")
                output = {"status": "success", "local_path": str(local_path), "vm_name": self.args.vm_name, "disk_key": disk.key, "datastore": datastore, "ds_path": ds_path}
                if self.args.json:
                    print(json.dumps(output, indent=2))
                else:
            ...
            if action == "cbt_sync":
                ...
                try:
                    device_key = disk.key
                    changed = client.query_changed_disk_areas(vm, snapshot=snap, device_key=device_key, start_offset=0, change_id=self.args.change_id)
                    if not getattr(changed, "changedDiskAreas", None):
                        ...
                        url = f"https://{vc_host}/folder/{ds_path}?dcPath={self.args.dc_name}&dsName={datastore}"
