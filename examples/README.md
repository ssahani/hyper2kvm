hyper2kvm YAML Examples Pack
==========================

These examples are designed for the hyper2kvm CLI + two-phase config parsing:
...
Pick one example and run it like:
  ...
  sudo ./hyper2kvm.py --config examples/<file>.yaml <command>

Or merge defaults + overrides:

  sudo ./hyper2kvm.py --config examples/00-common/common.yaml --config examples/10-local/local-linux-basic.yaml local
...
```bash
sudo python hyper2kvm.py --config examples/yaml/10-local/local-linux-basic.yaml
sudo python hyper2kvm.py --config examples/json/10-local/local-linux-basic.json
```
