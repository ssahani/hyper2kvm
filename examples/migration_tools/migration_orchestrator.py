...
class MigrationOrchestrator:
    ...
    def execute(self) -> dict[str, Any]:
        ...
        try:
            ...
            self._execute_phase(
                MigrationPhase.READINESS_ASSESSMENT,
                self._phase_readiness_assessment
            )
            ...
            self._execute_phase(
                MigrationPhase.PRE_MIGRATION_BACKUP,
                self._phase_pre_migration_backup
            )

            # Phase 3: Inspection
            self._execute_phase(
                MigrationPhase.INSPECTION,
                self._phase_inspection
            )

            # Phase 4: Migration
            self._execute_phase(
                MigrationPhase.MIGRATION,
                self._phase_migration
            )

            # Strategy-specific phases
            if self._should_execute_phase("service_management"):
                self._execute_phase(
                    MigrationPhase.SERVICE_MANAGEMENT,
                    self._phase_service_management
                )

            if self._should_execute_phase("network_configuration"):
                self._execute_phase(
                    MigrationPhase.NETWORK_CONFIGURATION,
                    self._phase_network_configuration
                )

            if self._should_execute_phase("security_hardening"):
                self._execute_phase(
                    MigrationPhase.SECURITY_HARDENING,
                    self._phase_security_hardening
                )

            if self._should_execute_phase("boot_validation"):
                self._execute_phase(
                    MigrationPhase.BOOT_VALIDATION,
                    self._phase_boot_validation
                )

            # Phase 5: Post-migration Validation
            self._execute_phase(
                MigrationPhase.POST_MIGRATION_VALIDATION,
                self._phase_post_migration_validation
            )
            ...
            self._execute_phase(
                MigrationPhase.FINAL_REPORT,
                self._phase_final_report
            )
    ...
    def _should_execute_phase(self, phase_name: str) -> bool:
        ...
        strategy_phases = {
            MigrationStrategy.BASIC: [
                "inspection", "migration"
            ],
            MigrationStrategy.ENTERPRISE: [
                "inspection", "migration", "service_management",
                "network_configuration", "security_hardening",
                "boot_validation"
            ],
            MigrationStrategy.DATABASE: [
                "inspection", "migration", "service_management",
                "boot_validation"
            ],
            ...
            MigrationStrategy.MINIMAL_DOWNTIME: [
                "inspection", "migration", "service_management"
            ]
        }
    ...
    def _phase_security_hardening(self) -> dict[str, Any]:
        ...
        with VMCraft(str(self.target_disk)) as g:
            ...
            security_result = {
                ...
                "success": len(hardening_applied) > 0
            }
    ...
    def _check_validation_passed(self) -> bool:
        ...
        critical_issues = [
            issue for issue in self.validation_report.get("issues", [])
...
Examples:
  ...
  %(prog)s batch-migrate migrations.json
    ...
    migrate_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Dry-run mode (assessment only, no migration)"
    )
    migrate_parser.add_argument(
        "--phases",
        help="Comma-separated list of custom phases to execute"
    )
    migrate_parser.add_argument(
        "--verbose",
        "-v",
