...
class PostMigrationValidation:
        ...
        try:
            ...
            try:
                ...
                if ifcfg_count > 0:
                    ...
                    self.remediation_steps.append("Review and remove old ifcfg files if using systemd-networkd")
            except:
                pass
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        print(f"Production Score: {self.production_score}/100")
        print(f"Readiness: {self.validations.get('production_readiness', {}).get('readiness_level', 'UNKNOWN')}")

        # Validation breakdown
        pass_count = sum(1 for v in self.validations.values() if isinstance(v, dict) and v.get('status') == 'PASS')
        warn_count = sum(1 for v in self.validations.values() if isinstance(v, dict) and v.get('status') == 'WARN')
        fail_count = sum(1 for v in self.validations.values() if isinstance(v, dict) and v.get('status') == 'FAIL')
        skip_count = sum(1 for v in self.validations.values() if isinstance(v, dict) and v.get('status') == 'SKIP')
        ...
        report = {
            ...
            'production_score': self.production_score,
            'readiness_level': self.validations.get('production_readiness', {}).get('readiness_level', 'UNKNOWN'),
