...
class MigrationAnalytics:
    ...
    def add_migration_report(self, report_path: Path) -> bool:
        ...
        try:
            ...
            cursor = self.conn.cursor()
            cursor.execute(
                "SELECT id FROM migrations WHERE migration_id = ?",
                (migration_id,)
            )
            if cursor.fetchone():
    ...
    def get_trends(self, days: int = 30) -> dict[str, Any]:
        ...
        success_trend = [
            {
                "date": row["day"],
                "success_rate": round(row["successful"] / row["total"] * 100, 2)
            }
            for row in cursor.fetchall()
        ]
        ...
        """, (cutoff_date,))
        duration_trend = [
            {"date": row["day"], "avg_duration": round(row["avg_duration"], 2)}
            for row in cursor.fetchall()
        ]
