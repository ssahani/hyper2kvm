...
class ComplianceReportGenerator:
    ...
    def generate_markdown_report(self, result: ComplianceResult) -> str:
        ...
        failed_low = [c for c in result.checks if not c.passed and c.level == ComplianceLevel.LOW]
        ...
        if failed_critical:
            ...
            for check in failed_critical:
                ...
                if check.references:
                    lines.extend([
                        f"**References**: {', '.join(check.references)}",
                        ""
                    ])
                lines.append("---")
                lines.append("")
        ...
        if passed_checks:
            lines.extend([
                "## ✅ Passed Checks",
                "",
                f"{len(passed_checks)} checks passed successfully:",
                ""
            ])
    ...
    def generate_csv_report(self, result: ComplianceResult) -> str:
        ...
        """
        lines = [
            "Check ID,Title,Level,Status,Finding,Remediation"
        ]
