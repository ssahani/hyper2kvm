...
class ComplianceOrchestrator:
    ...
    def get_change_tracker(self, vm_name: str, output_dir: Path) -> ChangeTracker:
        ...
        if vm_name not in self.change_trackers:
            self.change_trackers[vm_name] = ChangeTracker(
                self.logger,
                output_dir,
                vm_name
            )
        ...
        for framework in frameworks:
            ...
            if self.audit_logger:
                self.audit_logger.log_event(
                    event_type=__import__('hyper2kvm.compliance.audit_logger', fromlist=['AuditEventType']).AuditEventType.COMPLIANCE_SCAN_STARTED,
                    vm_name=vm_name,
    ...
    def generate_reports(
        ...
        for framework, result in results.items():
            ...
            files = self.report_generator.save_reports(
                result,
                output_dir,
                formats
            )
    ...
    def get_compliance_summary(
        ...
        if summary["aggregate"]["total_checks"] > 0:
            summary["aggregate"]["compliance_score"] = (
                summary["aggregate"]["passed_checks"] /
                summary["aggregate"]["total_checks"]
            ) * 100.0
        else:
    ...
    def full_compliance_workflow(
        ...
        try:
            ...
            result["reports_generated"] = {
                fmt: [str(f) for f in files]
                for fmt, files in report_files.items()
            }
