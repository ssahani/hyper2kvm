...
class ActiveDirectoryManager:
    ...
    def detect_domain_membership(self, g: VMCraft) -> dict[str, Any]:
        ...
        try:
            ...
            if audit["is_domain_joined"]:
                self.logger.info(
                    f"Detected domain membership: {computer_name} in {audit['domain_name']}"
                )
            else:
    ...
    def inject_domain_rejoin_script(
        ...
        try:
            ...
            audit["error"] = str(e)
            self.logger.error(f"Failed to inject domain rejoin script: {e}")
...
"""
            ...
            audit["error"] = str(e)
            self.logger.error(f"Failed to generate cleanup script: {e}")
...
try {{
    ...
    {'''if ($CurrentDomain -ne "WORKGROUP") {
        ...
        exit 0
    }''' if force_rejoin else '# Force rejoin disabled'}
