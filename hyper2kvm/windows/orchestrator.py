...
class WindowsMigrationOrchestrator:
    ...
    def detect_windows_configuration(self, g: VMCraft) -> dict[str, Any]:
        ...
        try:
            ...
            config["error"] = str(e)
            self.logger.error(f"❌ Windows configuration detection failed: {e}")
    ...
    def prepare_windows_migration(
        ...
        try:
            ...
            if config["license"].get("detected"):
                ...
                self._prepare_license_reactivation(
                    g, config["license"], scripts_dir, audit
                )
            ...
            if config["sql_server"].get("detected"):
                ...
                self._prepare_sql_server(
                    g, config["sql_server"], scripts_dir, audit
                )
            ...
            audit["error"] = str(e)
            self.logger.error(f"❌ Windows VM preparation failed: {e}")
    ...
    def _prepare_domain_rejoin(
        ...
        try:
            ...
            script_content = self.ad_mgr.create_domain_rejoin_script(
                domain_info, credentials=credentials
            )
            ...
            # Inject script
            inject_result = self.ad_mgr.inject_domain_rejoin_script(
                g, script_content, run_on_boot=True
            )
            ...
            if credentials:
                cleanup_result = self.ad_mgr.cleanup_old_computer_object(
                    domain_info, credentials
                )

                if cleanup_result.get("script_generated"):
                    cleanup_path = scripts_dir / "ad-cleanup.ps1"
                    cleanup_path.write_text(
                        cleanup_result["script_content"], encoding="utf-8"
                    )
                    ...
                    instructions_path.write_text(
                        cleanup_result["instructions"], encoding="utf-8"
                    )
    ...
    def _prepare_sql_server(
        ...
        try:
            ...
            result = self.sql_mgr.migrate_sql_configuration(
                g, sql_info, target_ip, target_hostname
            )
    ...
    def _log_detection_summary(
        ...
        if license_info.get("detected"):
            self.logger.info(
                f"  License: {license_info.get('license_type')} "
                f"({license_info.get('product_name')})"
            )
            self.logger.info(
                f"  Activation: {license_info.get('activation_status')}"
            )
        else:
        ...
        if domain_info.get("is_domain_joined"):
            self.logger.info(
                f"  Domain: {domain_info.get('computer_name')} in "
                f"{domain_info.get('domain_name')}"
            )
        else:
        ...
        if sql_info.get("detected"):
            ...
            self.logger.info(
                f"  SQL Server: {len(instances)} instance(s), "
...
"""
        ...
        guide = guide.replace("{timestamp}", datetime.datetime.now().isoformat())
