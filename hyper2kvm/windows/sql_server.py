...
class SQLServerManager:
    ...
    def migrate_sql_configuration(
        ...
        try:
            ...
            script = self._generate_sql_migration_script(
                instances, target_ip, target_hostname
            )
            ...
            self.logger.info(
                f"Generated SQL Server migration script for {len(instances)} instance(s)"
            )

        except Exception as e:
            audit["error"] = str(e)
            self.logger.error(f"Failed to create SQL migration script: {e}")
    ...
    def create_sql_migration_script(
        ...
        """
        return self._generate_sql_migration_script(
            sql_info.get("instances", []), target_ip, target_hostname
        )
    ...
    def _detect_instances(self, g: VMCraft) -> list[dict[str, Any]]:
        ...
        try:
            ...
            default_service = "/Windows/System32/config/systemprofile/AppData/Local/Microsoft/Microsoft SQL Server"
...
try {{
    # New server details
    {f'$NewIP = "{target_ip}"' if target_ip else '$NewIP = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object {$_.PrefixOrigin -ne "WellKnown"} | Select-Object -First 1).IPAddress'}
...
{instances_config}
    ...
    {chr(10).join([f'    Restart-Service "MSSQL${inst["name"]}" -Force' if inst["name"] != "MSSQLSERVER" else '    Restart-Service "MSSQLSERVER" -Force' for inst in instances])}
...
{chr(10).join([f'''
        $ServerInstance = if ("{inst["name"]}" -eq "MSSQLSERVER") {{ "localhost" }} else {{ "localhost\\{inst["name"]}" }}
        ...
        $Uri = "ManagedComputer[@Name='$env:COMPUTERNAME']/ServerInstance[@Name='{inst["name"]}']/ServerProtocol[@Name='Tcp']"
        $Tcp = $Wmi.GetSmoObject($Uri)
        $Tcp.IsEnabled = $true
        $Tcp.Alter()
        Write-Log "TCP/IP protocol enabled for {inst["name"]}"
''' for inst in instances])}
...
{chr(10).join([f'''
    $Instance = if ("{inst["name"]}" -eq "MSSQLSERVER") {{ "localhost" }} else {{ "localhost\\{inst["name"]}" }}
