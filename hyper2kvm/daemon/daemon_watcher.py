...
class VMFileHandler(FileSystemEventHandler):
    ...
    def _queue_file(self, path: Path) -> None:
        ...
        if self.deduplicator:
            ...
            if duplicate_info:
                self.logger.info(f"⏭️ Skipping duplicate: {path.name} "
...
class DaemonWatcher:
    ...
    def _process_file(self, file_path: Path, retry_count: int = 0) -> None:
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to process {file_path.name}: {error_message}")
    ...
    def _check_stalled(self) -> None:
        ...
        if idle_minutes > stall_threshold_minutes:
            ...
            if queue_depth > 0:
                self.logger.warning(f"⚠️ Daemon may be stalled: {queue_depth} items in queue, "
