...
class DaemonStatistics:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"📊 Loaded stats: {self.total_processed} processed, {self.total_failed} failed")
    ...
    def job_completed(self, filename: str, success: bool, error: str | None = None) -> None:
        ...
        with self.lock:
            ...
            self.logger.info(f"📊 Job {'completed' if success else 'failed'}: {filename} ({job.duration_seconds:.1f}s)")
