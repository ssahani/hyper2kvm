...
class SystemdNetworkdManager:
    ...
    def create_netdev_file(
        ...
        try:
            ...
            # Construct file path
            if not name.endswith('.netdev'):
                filename = f"10-{name}.netdev"
            else:
                filename = name
    ...
    def create_bridge_network(
        ...
        try:
            ...
            netdev_result = self.create_netdev_file(
                name=bridge_name,
                kind="bridge",
                netdev_config={}
            )
            ...
            bridge_network = self.create_network_file(
                name=f"10-{bridge_name}",
                match={"Name": bridge_name},
                network={},
                dhcp="yes"
            )
