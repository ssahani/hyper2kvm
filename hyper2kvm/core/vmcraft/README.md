...
VMCraft is a pure Python library for VM disk image manipulation, providing a drop-in replacement for libguestfs with native Linux tools.
...
VMCraft maintains API compatibility with libguestfs:
...
VMCraft is significantly faster than libguestfs:
- **NBD connection**: ~1.4s (vs libguestfs appliance boot: ~8-10s)
- **Inspection**: ~0.3s (vs libguestfs: ~2-3s)
- **Total launch time**: ~1.9s (vs libguestfs: ~10-13s)
...
- [libguestfs Documentation](http://libguestfs.org/) - Original API reference
