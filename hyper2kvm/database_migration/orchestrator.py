...
class DatabaseMigrationOrchestrator:
    ...
    def __init__(self, logger: logging.Logger):
        ...
        """
        self.logger = logger
        self.detector = DatabaseDetector(logger)
    ...
    def detect_databases(self, vmcraft_instance) -> list[DatabaseInfo]:
        ...
        if databases:
            ...
            for db in databases:
                self.logger.info(
                    f"  - {db.engine.value} {db.version} "
                    f"({db.instance_name or 'default'})"
                )
        else:
    ...
    def pre_migration_checks(self, databases: list[DatabaseInfo]) -> dict[str, Any]:
        ...
        for db_info in databases:
            ...
            if not check_result["healthy"]:
                ...
                results["critical_errors"].extend([
                    f"{db_info.instance_name}: {err}"
                    for err in check_result["errors"]
                ])

            results["warnings"].extend([
                f"{db_info.instance_name}: {warn}"
        ...
        if results["all_healthy"]:
            ...
            self.logger.warning(
                f"⚠ {len(results['critical_errors'])} critical error(s) found"
            )
        ...
        for db_info in databases:
            ...
            if not quiesce_result["success"]:
                ...
                results["errors"].extend([
                    f"{db_info.instance_name}: {err}"
                    for err in quiesce_result["errors"]
                ])

        if results["success"]:
            self.logger.info(
                f"✓ All databases quiesced "
                f"(total: {results['total_duration_seconds']:.1f}s)"
            )
        else:
    ...
    def validate_post_migration(
        ...
        if results["all_valid"]:
            ...
            self.logger.warning(
                f"⚠ {len(results['errors'])} validation error(s) found"
            )
    ...
    def tune_for_kvm(self, databases: list[DatabaseInfo]) -> dict[str, Any]:
        ...
        self.logger.info(
            f"Generated {len(results['all_recommended'])} tuning recommendation(s)"
        )
    ...
    def orchestrate_migration(
        ...
        try:
            ...
            self.logger.error(f"Migration workflow failed: {e}")
