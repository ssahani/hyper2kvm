...
def detect_hardware_dependent_apps(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                for uninstall_path in uninstall_paths:
                    findings.extend(
                        _scan_uninstall_key(hive, uninstall_path)
                    )
        ...
        logger.error(f"Failed to scan for hardware-dependent apps: {e}")
        logger.debug("App detection error", exc_info=True)

    logger.info(f"Found {len(findings)} hardware-dependent applications")
    return findings
...
def _scan_uninstall_key(hive: "hivex.Hivex", uninstall_path: str) -> List[AppCompatFinding]:
    ...
    try:
        ...
        logger.debug(f"Error scanning Uninstall key {uninstall_path}: {e}")
        ...
        try:
            ...
            if value:
                ...
                if val_type in [1, 2]:  # REG_SZ or REG_EXPAND_SZ
                    try:
                        app_info[value_name] = val_data.decode(
...
def detect_license_services(
    ...
    try:
        ...
        logger.error(f"Failed to scan for license services: {e}")
        logger.debug("License service detection error", exc_info=True)

    logger.info(f"Found {len(findings)} license manager services")
    return findings
    ...
    try:
        for driver_name, driver_info in DONGLE_DRIVERS.items():
            ...
            try:
                ...
                logger.debug(f"Error checking for driver {driver_name}: {e}")

    except Exception as e:
        logger.error(f"Failed to scan for dongle drivers: {e}")
        logger.debug("Dongle driver detection error", exc_info=True)

    logger.info(f"Found {len(findings)} dongle drivers")
    return findings
