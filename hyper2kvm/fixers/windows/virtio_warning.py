...
def warn_no_virtio_drivers(logger: logging.Logger, windows_info: Dict[str, Any] = None) -> None:
    """
    ...
    logger.warning("  1. Download latest VirtIO drivers:")
    logger.warning("       https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/")
    logger.warning("")
    logger.warning("  2. Download either:")
    logger.warning("       • virtio-win.iso  (recommended - includes all drivers)")
    logger.warning("       • virtio-win-<version>.zip  (extract to directory)")
    logger.warning("")
    logger.warning("  3. Re-run migration with VirtIO drivers:")
    logger.warning("       hyper2kvm local \\")
    logger.warning("         --vmdk your-vm.vmdk \\")
    logger.warning("         --virtio-drivers /path/to/virtio-win.iso \\")
    logger.warning("         --to-output output.qcow2")
    logger.warning("")
...
def get_virtio_download_url(windows_version: str = None) -> str:
    """
...
def should_warn_about_virtio(virtio_drivers_dir: str = None, quiet: bool = False) -> bool:
    """
