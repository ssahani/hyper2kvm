...
class FstabEntry:
    ...
    def __str__(self) -> str:
        ...
        return f"{self.device}\t{self.mountpoint}\t{self.fstype}\t{self.options}\t{self.dump}\t{self.pass_num}"
...
class FstabFixer:
    ...
    def fix(self) -> Dict[str, any]:
        ...
        """
        self.logger.info(f"Starting fstab fix: mode={self.mode.value}, dry_run={self.dry_run}")
        ...
        try:
            ...
            self._parse_fstab()
            self.stats['total_entries'] = len([e for e in self.entries if not e.is_comment and not e.is_blank])
            ...
            self.logger.info(
                f"fstab fix complete: converted={self.stats['converted']}, "
                f"removed={self.stats['removed']}, warnings={self.stats['warnings']}"
            )
    ...
    def _parse_fstab(self):
            ...
            for line_num, line in enumerate(f, start=1):
                ...
                if not original_line.strip():
                    self.entries.append(FstabEntry(
                        ...
                        device="", mountpoint="", fstype="", options="", dump="", pass_num="",
                ...
                if len(parts) < 4:
                    ...
                    self.logger.warning(f"Line {line_num}: Malformed fstab entry (< 4 fields)")
                    self.entries.append(FstabEntry(
        ...
        self.logger.debug(f"Parsed {len(self.entries)} fstab lines")
    ...
    def _resolve_devices(self):
        ...
        for entry in self.entries:
            ...
            if device_path:
                ...
                self.logger.debug(
                    f"Line {entry.line_number}: {entry.device} → {device_path} "
                    f"(UUID={info.uuid}, LABEL={info.label})"
                )
            else:
                self.logger.warning(
                    f"Line {entry.line_number}: Could not resolve device: {entry.device}"
                )
                self.stats['warnings'] += 1
    ...
    def _determine_fixes(self):
        ...
        for entry in self.entries:
            ...
            if entry.entry_type == FstabEntryType.VMWARE:
                ...
                self.logger.info(f"Line {entry.line_number}: Removing VMware entry: {entry.device}")
                continue
            ...
            if self.mode == FstabMode.UUID:
                if not entry.device.startswith('UUID='):
                    if entry.resolved_uuid:
                        ...
                        self.logger.info(f"Line {entry.line_number}: {entry.fix_reason}")
                    else:
                        warning = f"No UUID available for {entry.device} (keeping as-is)"
                        entry.fix_reason = warning
                        self.logger.warning(f"Line {entry.line_number}: {warning}")
                        self.stats['warnings'] += 1

            elif self.mode == FstabMode.LABEL:
                if not entry.device.startswith('LABEL='):
    ...
    def _write_fstab(self):
        ...
        self.logger.info(f"Wrote fixed fstab to {self.fstab_path}")
