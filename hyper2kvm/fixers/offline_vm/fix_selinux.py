...
from .utils import run_command, detect_os_from_root
...
class SELinuxFixer:
    ...
    def __init__(
        ...
        logger.info(f"SELinuxFixer initialized (root={root_mount}, dry_run={dry_run})")
    ...
    def fix(self) -> Dict[str, any]:
        ...
        try:
            ...
            logger.info(f"  OS: {self.os_info['family']} {self.os_info['version']}")
            ...
            logger.info(f"  SELinux mode: {self.selinux_info.mode.value}")

            if self.selinux_info.policy:
                logger.info(f"  SELinux policy: {self.selinux_info.policy}")
            ...
            return {
                ...
                'autorelabel_path': str(self.selinux_info.autorelabel_path) if self.selinux_info.autorelabel_path else None
            }

        except Exception as e:
            logger.error(f"SELinux fix failed: {e}", exc_info=True)
            self.stats['errors'] += 1
            return {
                'success': False,
                'error': str(e),
    ...
    def _detect_selinux(self) -> SELinuxInfo:
        ...
        if config_path.exists():
            logger.debug(f"Reading SELinux config: {config_path}")
        ...
        if policy:
            ...
            logger.debug(f"Policy directory exists: {policy_exists} ({policy_dir})")
    ...
    def _touch_autorelabel(self) -> bool:
        ...
        if not autorelabel_path:
            raise ValueError("autorelabel_path not set")

        # Check if already exists
        if autorelabel_path.exists():
            logger.debug(f"/.autorelabel already exists: {autorelabel_path}")
        ...
        if not self.dry_run:
            ...
            logger.debug(f"Created /.autorelabel: {autorelabel_path}")
        else:
            logger.info(f"[DRY-RUN] Would touch: {autorelabel_path}")
    ...
    def _validate_selinux(self):
        ...
        if not self.dry_run:
            if not self.selinux_info.autorelabel_path.exists():
