...
class GrubFixer:
    ...
    def fix(self) -> Dict[str, any]:
        ...
        """
        self.logger.info(f"Starting GRUB fix: boot_disk={self.boot_disk}, dry_run={self.dry_run}")
        ...
        try:
            ...
            self.logger.info(
                f"Detected: OS={self.os_info['family']}, "
                f"boot_mode={self.boot_mode.value}, "
                f"grub={self.grub_version.value}"
            )
            ...
            if not self.skip_install:
                ...
                if not self.dry_run:
                    ...
                    self.logger.info(f"DRY RUN: Would install GRUB to {self.boot_disk}")
            else:
            ...
            self.logger.info(
                f"GRUB fix complete: config={self.stats['config_regenerated']}, "
                f"installed={self.stats['grub_installed']}"
            )
    ...
    def _detect_grub_version(self) -> GrubVersion:
    ...
    def _prepare_grub_environment(self):
        ...
        if self.boot_mode == BootMode.BIOS:
            ...
            if not grub2_dir.exists() and not grub_dir.exists():
                ...
                if self.grub_version == GrubVersion.GRUB2:
                    ...
                    self.logger.info(f"Created {grub2_dir}")
                else:
                    grub_dir.mkdir(parents=True, exist_ok=True)
                    self.logger.info(f"Created {grub_dir}")
        ...
        # For UEFI systems, ensure /boot/efi/EFI exists
            ...
            if not efi_dir.exists():
                ...
                self.logger.info(f"Created {efi_dir}")
    ...
    def _regenerate_grub2_config(self):
        ...
        if config_path.exists():
            ...
            self.logger.info(f"Backed up GRUB config: {backup}")
        ...
        if self.os_info.get('family') in ['centos', 'rhel', 'fedora']:
            ...
            update_grub = self.root_mount / "usr/sbin/update-grub"
            if update_grub.exists():
                # update-grub is simpler
                cmd = ["update-grub"]
            else:
                cmd = ["grub-mkconfig", "-o", str(config_path)]

        # Run in chroot
        chroot_cmd = ["chroot", str(self.root_mount)] + cmd

        self.logger.debug(f"Running: {' '.join(chroot_cmd)}")
        ...
        try:
            ...
            if stdout:
                self.logger.debug(f"grub-mkconfig output: {stdout[:500]}")

            self.logger.info(f"Regenerated GRUB config: {config_path}")

        except Exception as e:
            self.logger.error(f"Failed to regenerate GRUB config: {e}")
            self.stats['errors'] += 1
            raise
        ...
        try:
            ...
            if returncode != 0:
                ...
                if "error" in stderr.lower():
                    raise RuntimeError(f"grub-install failed: {stderr}")
                else:
                    self.logger.warning(f"grub-install warnings: {stderr}")

            if stdout:
                self.logger.debug(f"grub-install output: {stdout}")

            self.logger.info(f"Installed GRUB to {self.boot_disk}")

        except Exception as e:
            self.logger.error(f"Failed to install GRUB: {e}")
            self.stats['errors'] += 1
            raise
    ...
    def _validate_grub_config(self):
        ...
        for config_path in config_paths:
            if config_path.exists():
                ...
                if all_good:
                    self.logger.info(f"✓ GRUB config validation passed: {config_path.name}")
                else:
