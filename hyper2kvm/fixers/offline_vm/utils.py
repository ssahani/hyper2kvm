...
def get_block_device_uuid(device: str) -> Optional[str]:
    ...
    try:
        ...
        if returncode == 0 and stdout:
            uuid = stdout.strip()
            logger.debug(f"Found UUID for {device}: {uuid}")
            return uuid

        logger.debug(f"No UUID found for {device}")
        return None

    except Exception as e:
        logger.warning(f"Failed to get UUID for {device}: {e}")
        return None
...
def get_block_device_label(device: str) -> Optional[str]:
    ...
    try:
        ...
        if returncode == 0 and stdout:
            label = stdout.strip()
            logger.debug(f"Found LABEL for {device}: {label}")
            return label

        return None

    except Exception as e:
        logger.warning(f"Failed to get LABEL for {device}: {e}")
        return None
...
def get_block_device_fstype(device: str) -> Optional[str]:
    ...
    try:
        ...
        if returncode == 0 and stdout:
            fstype = stdout.strip()
            logger.debug(f"Found TYPE for {device}: {fstype}")
            return fstype

        return None

    except Exception as e:
        logger.warning(f"Failed to get TYPE for {device}: {e}")
        return None
...
def resolve_device_to_path(device_spec: str, root_mount: str = "/mnt") -> Optional[str]:
    """
    ...
    Args:
        ...
        root_mount: Root filesystem mount point (for checking /dev)
...
def detect_os_from_root(root_mount: str) -> Dict[str, str]:
    ...
    if not os_release_path.exists():
        ...
        logger.warning(f"No /etc/os-release found in {root_mount}")
        return {
            'family': 'unknown',
            'name': 'Unknown',
            'version': 'unknown',
