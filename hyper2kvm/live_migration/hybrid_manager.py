...
class HybridMigrationManager:
    ...
    async def migrate_hybrid(
        ...
        try:
            ...
            self.logger.info(
                f"Live migration completed - Downtime: {live_result['actual_downtime_ms']}ms"
            )
            ...
            if offline_fixes:
                if maintenance_window:
                    ...
                    fixes_result = await self._apply_offline_fixes(
                        vm_id, offline_fixes, progress_callback
                    )
                ...
                result["total_downtime_seconds"] = (
                    live_result["actual_downtime_ms"] / 1000.0
                )
        ...
        try:
            ...
            for idx, fix in enumerate(offline_fixes):
                ...
                try:
                    ...
                    self.logger.error(f"Fix failed: {fix} - {e}")
    ...
    def estimate_hybrid_migration_time(
        ...
        estimate["total_seconds"] = (
            estimate["live_migration_seconds"] + estimate["offline_fixes_seconds"]
        )
