...
class LiveMigrationAnalyzer:
    ...
    def can_migrate_live(self, vm_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if analysis["recommended"]:
                ...
                analysis["reasons"].append(
                    "Offline migration recommended for this VM configuration"
                )
            ...
            self.logger.error(f"Live migration analysis failed: {e}")
    ...
    def _check_blockers(self, vm_info: dict[str, Any]) -> list[str]:
        ...
        if snapshot_count > 0:
            blockers.append(
                f"VM has {snapshot_count} snapshot(s) - must consolidate before live migration"
            )

        # Check for connected devices (USB, CD-ROM, etc.)
        connected_devices = vm_info.get("connected_devices", [])
        blocking_devices = [
            d for d in connected_devices if d.get("type") in ("usb", "cdrom", "floppy")
        ]
        if blocking_devices:
            device_types = ", ".join([d.get("type", "unknown") for d in blocking_devices])
            blockers.append(
                f"Connected devices blocking live migration: {device_types}"
            )

        # Check guest tools
        if not vm_info.get("guest_tools_running", False):
            blockers.append(
                "Guest tools not running - required for reliable live migration"
            )
    ...
    def _estimate_downtime(self, vm_info: dict[str, Any]) -> float:
        ...
        if "windows" in os_type:
            os_factor = 1.5  # Windows tends to have higher memory churn
        else:
            os_factor = 1.0
    ...
    def _get_requirements(self, vm_info: dict[str, Any]) -> list[str]:
        ...
        disk_size_gb = vm_info.get("disk_size_gb", 0)
        requirements.append(
            f"Ensure target has at least {disk_size_gb:.1f}GB available storage"
        )

        # Memory requirements
        memory_mb = vm_info.get("memory_mb", 0)
        requirements.append(
            f"Ensure target has at least {memory_mb}MB available memory"
        )
    ...
    def _get_warnings(self, vm_info: dict[str, Any]) -> list[str]:
        ...
        if memory_mb > self.MEMORY_LARGE:
            warnings.append(
                f"Large memory allocation ({memory_mb}MB) may increase migration time"
            )
        ...
        if disk_count > 3:
            warnings.append(
                f"Multiple disks ({disk_count}) may increase migration complexity"
            )

        # Thick provisioning warning
        if vm_info.get("disk_provisioning") == "thick":
            warnings.append(
                "Thick-provisioned disks will require full disk transfer"
            )
    ...
    def analyze_batch(self, vms: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if results["total_vms"] > 0:
            results["live_feasible_pct"] = (
                results["live_feasible"] / results["total_vms"] * 100
            )
            results["live_recommended_pct"] = (
                results["live_recommended"] / results["total_vms"] * 100
            )
