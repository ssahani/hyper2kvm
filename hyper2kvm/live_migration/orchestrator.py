...
class LiveMigrationOrchestrator:
    ...
    async def migrate(
        ...
        try:
            self.logger.info(
                f"Starting migration orchestration: {vm_id} "
                f"({source_host} → {target_host}, mode: {mode})"
            )
            ...
            if mode == "auto":
                ...
                selected_mode = mode
                result["mode_recommended"] = analysis.get(
                    "recommended", "unknown"
                )
            ...
            self.logger.info(
                f"Migration mode selected: {selected_mode} "
                f"(recommended: {result['mode_recommended']})"
            )
            ...
            if result["success"]:
                self.logger.info(
                    f"Migration completed successfully: {vm_id} (mode: {selected_mode})"
                )
            else:
                self.logger.error(
                    f"Migration failed: {vm_id} - {migration_result.get('error')}"
                )
    ...
    async def _execute_offline_migration(
        ...
        self.logger.warning(
            "Offline migration selected - using traditional hyper2kvm workflow"
        )

        return result

    async def _get_vm_info(
        self, vm_id: str, host: str, provider: str
    ) -> dict[str, Any]:
        """
    ...
    async def plan_batch_migration(
        ...
        for vm_analysis in batch_analysis["vms"]:
            ...
            if analysis["recommended"]:
                plan["live_migration_candidates"].append(
                    {
                        "name": vm_name,
                        "estimated_downtime_seconds": analysis[
        ...
        plan["estimated_total_downtime_minutes"] = sum(
            vm["estimated_downtime_seconds"] / 60
            for vm in plan["live_migration_candidates"]
        )
