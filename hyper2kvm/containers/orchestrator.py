...
class ContainerExtractionOrchestrator:
        ...
        try:
            ...
            for container in containers:
                ...
                if container.env_vars:
                    cm_result = self.k8s_generator.generate_configmap(
                        container,
                        k8s_dir
                    )
                ...
                if container.volumes:
                    pvc_result = self.k8s_generator.generate_persistent_volume_claim(
                        container,
                        k8s_dir
                    )
                ...
                image_result = extractor.export_container_image(
                    vmcraft_instance,
                    container,
                    images_dir
                )
            ...
            result["success"] = True
            self.logger.info(
                f"Generated {len(result['manifests_generated'])} Kubernetes manifests"
            )

        except Exception as e:
            result["errors"].append(str(e))
            self.logger.error(f"Kubernetes extraction failed: {e}")
    ...
    def _extract_for_docker(
        ...
        try:
            ...
            compose_result = extractor.generate_docker_compose(
                containers,
                docker_dir
            )
            ...
            for container in containers:
                dockerfile_result = extractor.generate_dockerfile(
                    container,
                    docker_dir
                )
            ...
            result["errors"].append(str(e))
            self.logger.error(f"Docker extraction failed: {e}")
