...
from .base import BackupSource, BackupFormat, BackupVMInfo, RestoreProgress
...
class ProxmoxBackupSource(BackupSource):
    ...
    def __init__(
        ...
        """
        super().__init__(logger, backup_path)
    ...
    def list_vms(self, filters: dict[str, Any] | None = None) -> list[BackupVMInfo]:
        ...
        if not self._has_pbs_client:
            self.logger.warning("proxmox-backup-client not available, scanning datastore manually")
        ...
        try:
            ...
            self.logger.error(f"Failed to list PBS snapshots: {e}")

        self.logger.info(f"Found {len(vms)} VMs in PBS backup")
        return vms

    def _list_vms_manual(self, filters: dict[str, Any] | None) -> list[BackupVMInfo]:
        ...
        for vm_dir in vm_dirs:
            ...
            for snapshot_dir in vm_dir.glob("*"):
                ...
                try:
                    ...
                    backup_date = datetime.strptime(timestamp_str, "%Y-%m-%dT%H:%M:%SZ")
                    ...
                    self.logger.debug(f"Failed to parse snapshot {snapshot_dir}: {e}")
                    continue
    ...
    def get_vm_info(self, vm_identifier: str) -> BackupVMInfo:
        ...
        for vm in vms:
            if vm.vm_uuid == vm_identifier or vm.vm_name == vm_identifier:
                return vm

        raise ValueError(f"VM '{vm_identifier}' not found in PBS backup")
    ...
    def restore_vm(
        ...
        if not self._has_pbs_client:
            ...
            result["warnings"].append(
                "Install proxmox-backup-client to restore from PBS: "
        ...
        try:
            ...
            for idx, disk in enumerate(vm_info.disks):
                ...
                # Convert to qcow2
                self.logger.info(f"Converting to qcow2")
            ...
            self.logger.error(f"Failed to restore VM: {e}")
