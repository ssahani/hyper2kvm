...
class BackupRestoreOrchestrator:
    ...
    def add_backup_source(
        ...
        format_hint: BackupFormat | None = None,
        **kwargs: Any
    ) -> bool:
        """
        Add backup source.
        ...
        try:
            ...
            self.logger.error(f"Failed to add backup source {name}: {e}")
            return False
    ...
    def list_all_vms(self, filters: dict[str, Any] | None = None) -> dict[str, list[BackupVMInfo]]:
        ...
        for name, source in self._sources.items():
            try:
                ...
                self.logger.error(f"Failed to list VMs from {name}: {e}")
                results[name] = []
    ...
    def restore_vm(
        ...
        if source_name not in self._sources:
            raise ValueError(f"Backup source '{source_name}' not found")
    ...
    def verify_all_backups(self) -> dict[str, Any]:
        ...
        for source_name, source in self._sources.items():
            ...
            try:
                ...
                self.logger.error(f"Failed to verify backups in {source_name}: {e}")
    ...
    def generate_dr_test_plan(
        self,
        vms: list[tuple[str, BackupVMInfo]],
        test_env_capacity_gb: int = 1000
    ) -> dict[str, Any]:
        """
        ...
        for source_name, vm_info in sorted_vms:
            ...
            if used_capacity + vm_info.backup_size_gb <= test_env_capacity_gb:
                ...
                plan["warnings"].append(
                    f"VM {vm_info.vm_name} ({vm_info.backup_size_gb} GB) "
                    f"exceeds remaining capacity"
                )
