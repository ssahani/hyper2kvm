...
class VeeamBackupSource(BackupSource):
    ...
    def __init__(self, logger: logging.Logger, backup_path: Path | str):
        """
    ...
    def list_vms(self, filters: dict[str, Any] | None = None) -> list[BackupVMInfo]:
        ...
        for vbk_file in vbk_files:
            try:
                ...
                self.logger.debug(f"Failed to parse VBK file {vbk_file}: {e}")
                continue
    ...
    def get_vm_info(self, vm_identifier: str) -> BackupVMInfo:
        ...
        if not vms:
            raise ValueError(f"VM '{vm_identifier}' not found in Veeam backup")

        # Return most recent backup if multiple found
        return max(vms, key=lambda vm: vm.backup_date or datetime.min)
    ...
    def restore_vm(
        ...
        try:
            ...
            self.logger.info(f"Converting VMDK to qcow2")
    ...
    def _convert_vmdk_to_qcow2(
                ...
                if "(" in line and "%" in line:
                    ...
                    try:
                        ...
                        progress = RestoreProgress(
                            ...
                            processed_bytes=int(vmdk_path.stat().st_size * pct / 100),
                            progress_pct=pct,
                            speed_mbps=0.0,  # qemu-img doesn't provide speed
                            eta_seconds=None,
        ...
        if process.returncode != 0:
            raise RuntimeError(f"VMDK conversion failed: qemu-img exited with code {process.returncode}")
