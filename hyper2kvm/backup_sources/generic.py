...
class GenericBackupSource(BackupSource):
    ...
    def _detect_backup_type(self) -> str:
        ...
        if self.backup_path.is_dir():
            return "directory"
        elif self.backup_path.suffix in [".tar", ".tgz", ".tar.gz", ".tar.bz2", ".tar.xz"]:
            return "tar"
        elif self.backup_path.suffix == ".zip":
            return "zip"
        else:
            return "unknown"
    ...
    def _find_disk_images(self) -> list[dict[str, Any]]:
        ...
        if self.backup_type == "directory":
            ...
            try:
                ...
                self.logger.debug(f"Failed to list tar contents: {e}")
            ...
            try:
                ...
                self.logger.debug(f"Failed to list zip contents: {e}")
