...
from .schemas import JobState, JobSpec, JobResult
...
class JobStateMachine:
    ...
    def transition(self, to_state: JobState, reason: str = "") -> None:
        ...
        if not self.can_transition(to_state):
            raise InvalidStateTransition(
                self.current_state,
                to_state,
                reason or "Transition not allowed"
            )
    ...
    def from_dict(cls, data: Dict) -> "JobStateMachine":
        ...
        sm = cls(
            job_id=data["job_id"],
            initial_state=JobState(data["current_state"])
        )
    ...
    @classmethod
    def load(cls, job_id: str, state_dir: Path) -> Optional["JobStateMachine"]:
        """
...
class JobRegistry:
    ...
    def get(self, job_id: str) -> Optional[JobStateMachine]:
        """
