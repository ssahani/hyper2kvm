...
from hyper2kvm.worker.state_machine import JobRegistry, JobStateMachine
from hyper2kvm.worker.engine import WorkerEngine
from hyper2kvm.worker.events import EventStore, EventStream
from hyper2kvm.worker.scheduler import JobQueue, WorkerRegistry
from hyper2kvm.worker.capabilities import CapabilityDetector
from hyper2kvm.worker.metrics import WorkerMetrics
...
class JobStatusResponse(BaseModel):
    ...
    latest_event: Optional[ProgressEvent] = Field(
        None, description="Most recent progress event"
    )
...
async def submit_job(
    ...
    try:
        ...
        # Register job
        job_registry.register(job_spec.job_id, sm)

        # Optionally queue for worker assignment
        queue_position = None
        if queue:
            job_queue.enqueue(job_spec)
            sm.transition(JobState.QUEUED, f"Added to queue with priority {job_spec.execution_policy.priority}")
...
async def get_job_status(job_id: str) -> JobStatusResponse:
    ...
    from hyper2kvm.worker.schemas import ImageSpec, ExecutionPolicy
    job_spec = JobSpec(
        job_id=job_id,
...
async def list_jobs(
    ...
    for job_id in all_jobs:
        ...
        jobs.append({
            ...
            "created_at": sm.state_history[0].timestamp.isoformat() if sm.state_history else None,
...
async def get_job_events(
    ...
    try:
        ...
        events = event_store.get_events(job_id, since=since_dt)
        return events

    except Exception as e:
        logger.error(f"Error retrieving events for {job_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def stream_job_events(job_id: str, request: Request):
    ...
    async def event_generator():
        ...
        try:
            ...
            logger.error(f"Error in event stream for {job_id}: {e}")
            yield {
                "event": "error",
                "data": f'{{"error": "{str(e)}"}}',
            }
...
async def register_worker(capabilities: WorkerCapabilities) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Registered worker {capabilities.worker_id} with capabilities: {capabilities.capabilities}")
...
@app.post("/workers/{worker_id}/heartbeat", status_code=200)
...
async def dequeue_job(capabilities: WorkerCapabilities) -> Optional[JobSpec]:
    ...
    """
    try:
        job_spec = job_queue.dequeue(capabilities)
...
async def health_check() -> HealthResponse:
    ...
    active_jobs = len([
        j for j in all_jobs
        if not job_registry.get(j).is_terminal()
    ])
