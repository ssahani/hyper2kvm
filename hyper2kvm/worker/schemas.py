...
class ImageSpec(BaseModel):
    ...
    format: Optional[str] = Field(None, description="Image format (qcow2, vmdk, raw, etc.)")
    checksum: Optional[str] = Field(None, description="SHA-256 checksum for validation")
...
class CapabilityRequirements(BaseModel):
    ...
    min_memory_gb: Optional[int] = Field(None, description="Minimum RAM in GB", ge=1)
    min_disk_space_gb: Optional[int] = Field(None, description="Minimum free disk space in GB", ge=1)
...
class AuditInfo(BaseModel):
    ...
    ticket: Optional[str] = Field(None, description="Associated ticket ID (e.g., INC-1234)")
    tags: Optional[Dict[str, str]] = Field(None, description="Custom tags for job categorization")
...
class JobSpec(BaseModel):
    ...
    requested_actions: List[str] = Field(
        ...
        description="Specific actions to perform (e.g., nbd_attach, lvm_activate)"
    )
...
class ProgressEvent(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(None, description="Additional phase-specific details")
...
class JobOutput(BaseModel):
    ...
    fixed_image: Optional[str] = Field(None, description="Path to repaired image")
    logs: Optional[str] = Field(None, description="Path to execution logs")
    report: Optional[str] = Field(None, description="Path to detailed report")
    intermediates: Optional[List[str]] = Field(None, description="Paths to intermediate files")
...
class JobMetrics(BaseModel):
    ...
    nbd_attach_seconds: Optional[int] = Field(None, description="Time spent attaching NBD", ge=0)
    lvm_activate_seconds: Optional[int] = Field(None, description="Time spent activating LVM", ge=0)
    fsck_seconds: Optional[int] = Field(None, description="Time spent on filesystem check", ge=0)
    initramfs_seconds: Optional[int] = Field(None, description="Time spent regenerating initramfs", ge=0)

    disk_read_bytes: Optional[int] = Field(None, description="Bytes read from disk", ge=0)
    disk_write_bytes: Optional[int] = Field(None, description="Bytes written to disk", ge=0)
...
class JobResult(BaseModel):
    ...
    outputs: Optional[JobOutput] = Field(None, description="Output artifacts")
    metrics: Optional[JobMetrics] = Field(None, description="Execution metrics")
            ...
            "examples": [
                {
                    ...
                    "outputs": {
                        "fixed_image": "/mnt/output/vm-fixed.qcow2",
                        "logs": "/mnt/output/job.log"
                    },
                    "metrics": {
                        "execution_seconds": 3200,
                        "nbd_attach_seconds": 2,
                        "fsck_seconds": 900
                    },
                    "worker_id": "worker-01"
                },
...
class WorkerCapabilities(BaseModel):
    ...
    os_info: Optional[Dict[str, str]] = Field(None, description="OS information")
