...
class WorkerMetrics:
    ...
    def __init__(self, worker_id: str, enabled: bool = True):
        ...
        self.jobs_active = Gauge(
            'hyper2kvm_worker_jobs_active',
            'Number of currently active jobs',
            ['worker_id']
        )
        
        # VMDK size distribution
        self.vmdk_size = Histogram(
            'hyper2kvm_vmdk_size_bytes',
            'VMDK file size distribution',
            ['worker_id'],
            buckets=[
                1e9,      # 1 GB
                10e9,     # 10 GB
        ...
        self.temp_usage = Gauge(
            'hyper2kvm_conversion_temp_usage_bytes',
            'Temporary storage usage in bytes',
            ['worker_id']
        )
        
        self.temp_capacity = Gauge(
            'hyper2kvm_conversion_temp_capacity_bytes',
            'Temporary storage capacity in bytes',
            ['worker_id']
        )
