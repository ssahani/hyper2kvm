...
class MockOfflineFixRunner:
    ...
    def _simulate_operation(
        ...
        if operation == FixOperation.GRUB_REGENERATE:
        ...
        elif operation == FixOperation.INITRAMFS_REGENERATE:
            ...
            return FixOperationResult(
                ...
                details={
                    "tool": "dracut" if detected_os.family in [OSFamily.CENTOS, OSFamily.FEDORA] else "mkinitramfs",
                    "modules_added": ["virtio_blk", "virtio_scsi", "virtio_net", "virtio_pci"]
                }
            )

        elif operation == FixOperation.FSTAB_UUID:
            return FixOperationResult(
                operation=operation,
                result=FixResult.SUCCESS,
                ...
                details={
                    "entries_updated": 3,
                    "mode": request.parameters.fstab_mode
                }
            )

        elif operation == FixOperation.VIRTIO_INJECT:
            ...
            return FixOperationResult(
                ...
                details={
                    "drivers": ["virtio_blk", "virtio_scsi", "virtio_net"],
                    "method": "kernel_modules"
                }
            )

        elif operation == FixOperation.VMWARE_TOOLS_REMOVE:
        ...
        elif operation == FixOperation.SELINUX_RELABEL:
            ...
            return FixOperationResult(
                ...
                details={
                    "files_relabeled": 15234,
                    "policy": "targeted"
                }
            )

        elif operation == FixOperation.NETWORK_STABILIZE:
            return FixOperationResult(
                operation=operation,
                result=FixResult.SUCCESS,
    ...
    def _generate_warnings(
        self,
        operations: list[FixOperationResult],
        detected_os: DetectedOS
    ) -> list[str]:
    ...
    def _generate_recommendations(
        self,
        boot_confidence: BootConfidence,
        status: OfflineFixStatus
    ) -> list[str]:
