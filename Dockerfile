...
# Worker requires some privileged operations via sudo
RUN echo "hyper2kvm ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/hyper2kvm && \
...
RUN useradd -m -u 1000 -s /bin/bash hyper2kvm && \
    ...
    echo "hyper2kvm ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/hyper2kvm && \
    chmod 0440 /etc/sudoers.d/hyper2kvm
